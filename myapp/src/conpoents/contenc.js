import React from "react";
import { Installour } from "./howtoinstallourapp";
import Usersay from "./Userasay";
import Jointhefun from "./jointhefun";
import BetheReasone from "./bethereasone";
import EveryClick from "./everyClink";
import "./styles.css"


export function Content(){




    return(

             <div   style={{backgroundColor:"#F8F7F7" , marginTop:"10%"}}            >

                  <div>


                    <div      style={{display:"flex"   , justifyContent:"center"}}                              >

                       <div>
                         <p     style={{fontSize:"4rem" , fontStretch:"expanded" , fontWeight:"bold"}} >Features  That keep You Hooked</p>
                       </div>
                       
                    
                    </div>
                       <div    style={{display:"flex"   , justifyContent:"center"}}                  >
                         <p     style={{ fontStretch:"expanded" , fontWeight:"bold"}}>Meat. Chat .  Share - Anytime , Anywhere ! </p>
                       </div>



                    

            

                  </div>
         



                      



                      <div        style={{display:"flex" , justifyContent:"center"  , marginTop:"5%"}}>




                          <div   style={{width:"70%"   , display:"flex" , justifyContent:"space-around", flexFlow:"wrap"          }}  >

                       

                  
                               
                           
                               
                               <div  className="boxshadow"  style={{backgroundColor:"#F8F7F7"  ,  width:"300px" , height:"100px" , padding:"20px"  , margin:"0% 0% 3% 0%"  }} >
                                
                                
                                <div><p       style={{ fontStretch:"expanded" , fontWeight:"bold" , color:"blue"}}                                 >Seamless Connections</p>
                            
                            
                            </div>

                                  <div>



                                    
                                     <p>
                                    Stay  in touch  with friends family  and like-minded people with just a tap 
                                </p>
                                 </div>
                         
                        
                        
                          </div> 



                       






                         
                             <div  className="boxshadow" style={{backgroundColor:"#F8F7F7" ,  width:"300px", height:"100px" , padding:"20px"     }}        >
                                  
                                <div>
                                      <p      style={{ fontStretch:"expanded" , fontWeight:"bold" , color:"blue"}}                   >Share Your Story</p>
                                </div>
                             
                            <div>
                                  <p>
                                 Upload photos vedeos and updates to  let the world know what's happening  in your life 
                                </p>
                            </div>
                             </div> 



                             <div className="boxshadow"  style={{backgroundColor:"#F8F7F7" ,  width:"300px"  , height:"100px", padding:"20px"    }}        >
                               <div>   <p     style={{ fontStretch:"expanded" , fontWeight:"bold" , color:"blue"}}      >Real Time Chat</p></div>
                                 <div>
                                      <p>
                                   Whether  it's a DM or a group converstion connect instantly with smooth lightning last messaging
                                </p>
                                 </div>
                             </div>


                             <div  className="boxshadow"  style={{backgroundColor:"#F8F7F7"  ,  width:"300px" , height:"100px", padding:"20px"      }}    >
                         <div>
                                     <p           style={{ fontStretch:"expanded" , fontWeight:"bold" , color:"blue"}}         >Privacy First</p>
                         </div>
                              <div>
                                    <p>
                                   your data  your control we  prioritize your privacy with world class scurity
                                </p>
                              </div>
                             </div> 



                             <div className="boxshadow"  style={{backgroundColor:"#F8F7F7"   ,  width:"300px", height:"100px" , padding:"20px"     }}      >
                                 <div>
                                     <p       style={{ fontStretch:"expanded" , fontWeight:"bold" , color:"blue"}}                           >Discover & Explore</p>
                                 </div>
                            
                         <div>
                                <p>
                                    Find  trending  content join communities and follow pages that match your interest
                                </p>
                         </div>
                            
                             </div> 





                             <div className="boxshadow"    style={{backgroundColor:"#F8F7F7" , height:"100px" ,  width:"300px", padding:"20px"      }}         >
                           
                           
                           <div>
                                    <p  style={{ fontStretch:"expanded" , fontWeight:"bold", color:"blue"}}                                              >Grow Your bussiness</p>
                           </div>
                             
                       
                       
                       
                         <div>
                                 <p>
                                    Use our platform to market  your brand connect with customers and build meaningful relationships
                                </p>
                             
                         </div>
                             </div> 
                    

          </div>
                      </div>








                    </div>

         





    )
}